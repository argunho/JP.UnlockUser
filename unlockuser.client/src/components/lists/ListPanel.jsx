...
function ListPanel({ selected, ids, disabled, onSelected }) {
    ...
    return <div className={`ul-panel d-row jc-between w-100${checked ? " checked" : ""}`}>
        ...
        <Button disabled={selected?.length === 0 || disabled} onClick={onSelected} starIcon={<TuneSharp />}>
            Hantera {selected?.length} markerade
        </Button>
    </div>
}
